...
export default function App() {
  ...
  const handleAdminBookFilter = () => {
    ...
    // Checks Title, Author, Shelf, Category, Language, Volume, Section
    const result = books.filter(b => 
        (b.title && b.title.toLowerCase().includes(q)) ||
        (b.author && b.author.toLowerCase().includes(q)) ||
        (b.shelfNumber && b.shelfNumber.toLowerCase().includes(q)) ||
  ...
  const handleAdminMemberFilter = () => {
    ...
    // Checks Name, Phone, Father Name, Address
  ...
  const showIssuedBooks = () => {
    // Filter books where copies are 0 (Out of Stock / Issued)
    // Note: This is an approximation. Ideally we check active transactions.
    const result = books.filter(b => b.copies === 0);
    setDisplayedBooks(result);
    alert(`Showing ${result.length} Issued (Out of Stock) Books`);
  };
  ...
  const addBook = async () => {
    ...
    alert("Book Saved!"); setNewBook({ title: '', author: '', language: '', volume: '', section: '', category: '', shelfNumber: '', copies: 1 });
    refreshData();
  };
  ...
  const addMember = async () => {
    ...
    alert("Member Saved!"); setNewMember({ name: '', fatherName: '', address: '', email: '', phone: '' });
    refreshData();
  };

  const issueBook = async () => {
    if (!transBookId || !transMemberId) return alert("Enter IDs");
    try { await axios.post(`${API}/transactions/issue`, { bookId: transBookId, memberId: transMemberId }); alert("✅ Issued!"); refreshData(); }
    catch (e) { alert("Error: " + (e.response?.data?.error || "Failed")); }
  };

  const returnBook = async () => {
    if (!transBookId || !transMemberId) return alert("Enter IDs");
    try { await axios.post(`${API}/transactions/return`, { bookId: transBookId, memberId: transMemberId }); alert("✅ Returned!"); refreshData(); }
    catch (e) { alert("Error: " + (e.response?.data?.error || "Failed")); }
  };
